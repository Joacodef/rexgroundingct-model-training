...
OBJECTIVE:      Execute a fast 2-epoch diagnostic run of Exp 001 on data/dataset_mini.json,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            with torch.amp.autocast('cuda'):
                logits = model(images, text_embeds)
            ...
            if not is_finite_loss:
                ...
                logger.warning(
                    ...
                    f"BCE={loss_bce.item()}, Dice={loss_dice.item()}, Total={loss_val}, FG={fg_voxels}"
                )
            else:
                logger.info(
                    f"Epoch {epoch} Batch {batch_idx:02d} ({scan_id}): "
                    f"Loss={loss_val:.4f} (BCE={loss_bce.item():.4f}, Dice={loss_dice.item():.4f}), FG={fg_voxels}"
                )
...
def main() -> None:
    ...
    parser.add_argument("--dataset_json", type=str, default=str(ROOT_DIR / "data" / "dataset_mini.json"), help="Path to mini dataset.json")
