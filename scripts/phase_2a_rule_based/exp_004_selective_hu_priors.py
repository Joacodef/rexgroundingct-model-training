"""
===============================================================================
SCRIPT:         exp_004_selective_hu_priors.py
PHASE:          Phase 2A — Statistical / Rule-Based Prior Baseline
LOCATION:       scripts/phase_2a_rule_based/exp_004_selective_hu_priors.py
OBJECTIVE:      Single-file executable pipeline for Phase 2A Exp 004.
...
def parse_args():
    ...
    Objective:
        Parse command-line arguments for Exp 004 baseline pipeline.
    """
    parser = argparse.ArgumentParser(
        description="Phase 2A Exp 004: Selective HU Radiodensity Windowing + Quantile Spatial Prior Baseline Pipeline"
    )
    parser.add_argument(
...
def main():
    """Main CLI entry point for Exp 004 Pipeline."""
    args = parse_args()

    pdf_cache_path = Path(args.pdf_cache) if args.pdf_cache else DATA_DIR / "phase_2a" / "empirical_spatial_pdf_14cat_anchored.npz"
    output_dir = Path(args.output_dir) if args.output_dir else PREDICTIONS_DIR / "phase_2a_exp_004_selective_hu"
    exp_log_dir = LOGS_DIR / "phase_2a_rule_based" / "exp_004_selective_hu_priors"

    # Initialize Predictor Engine (Selective HU Windowing + Volume Quantile Matching Mode)
