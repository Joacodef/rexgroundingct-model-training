...
* **Universal Mask Policy & Symmetric On-Disk Storage**: Segmentation masks (both Ground Truth annotations and model predictions) conceptually lack independent physical orientation. All masks unconditionally inherit their parent CT scan's raw header affine (`RAW_IMAGES_DIR / nifti_path.name`). `save_nifti()` converts canonical `RAS` predictions back into the parent CT scan's raw voxel index space (`LPS`), saving prediction files on disk in the **100% identical format and header structure as official Ground Truth masks**. `load_nifti_ras()` requires zero optional arguments and canonicalizes all CT volumes and masks to `RAS` physical coordinates at load time.

### 5. Directory & Structure Conventions
* `scripts/`: Categorized into shared utilities (`common/`) and phase-specific execution pipelines (`phase_2a_rule_based/`, `phase_2b_voxtell_baseline/`, `phase_3_voxtell_finetuning/`, `phase_4_voxtell_spoco/`).
...
* **Relative Path Directive**: ALL documentation, markdown files, and codebase scripts MUST strictly use **relative paths** (e.g., `scripts/phase_2b_voxtell_baseline/exp_001_voxtell_inference.py`).
