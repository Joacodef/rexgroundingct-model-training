"""
===============================================================================
SCRIPT:         exp_003_hu_windowed_priors.py
PHASE:          Phase 2A — Statistical / Rule-Based Prior Baseline
LOCATION:       scripts/phase_2a_rule_based/exp_003_hu_windowed_priors.py
...
def parse_args():
    ...
    Objective:
        Parse command-line arguments for Exp 003 baseline pipeline.
    """
    parser = argparse.ArgumentParser(
        description="Phase 2A Exp 003: Radiodensity HU Intensity Windowing + Quantile Spatial Prior Baseline Pipeline"
    )
    parser.add_argument(
...
def main():
    """Main CLI entry point for Exp 003 Pipeline."""
    args = parse_args()

    pdf_cache_path = Path(args.pdf_cache) if args.pdf_cache else DATA_DIR / "phase_2a" / "empirical_spatial_pdf_14cat_anchored.npz"
    output_dir = Path(args.output_dir) if args.output_dir else PREDICTIONS_DIR / "phase_2a_exp_003_hu_windowed"
    exp_log_dir = LOGS_DIR / "phase_2a_rule_based" / "exp_003_hu_windowed_priors"

    # Initialize Predictor Engine (Radiodensity HU Windowing + Volume Quantile Matching Mode)
